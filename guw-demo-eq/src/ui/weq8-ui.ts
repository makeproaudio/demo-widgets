...
export class WEQ8UIElement extends LitElement {
  ...
  constructor() {
    ...
    this.runtime = new WEQ8Runtime(audioCtx);
    this.runtime.connect(audioCtx.destination);
    this.runtime.on("valueChange", (event) => {
      this.dispatchEvent(
  ...
  private _zoneActive: Zone = [];
  public get zoneActive(): Zone { return this._zoneActive; }
  @property({ type: Object })
  public set zoneActive(value: Zone) {
    this._zoneActive = value;
    for (let i = 0; i < value.length; i++) {
      this.runtime?.toggleBypass(i, !value[i].value);
    }
  }

  // do the same for type, frequency, gain, q
  private _zoneType: Zone = [];
  public get zoneType(): Zone { return this._zoneType; }
  @property({ type: Object })
  public set zoneType(value: Zone) {
    this._zoneType = value;
    for (let i = 0; i < value.length; i++) {
      this.runtime?.setFilterType(i, value[i].value);
    }
  }

  private _zoneFrequency: Zone = [];
  public get zoneFrequency(): Zone { return this._zoneFrequency; }
  @property({ type: Object })
  public set zoneFrequency(value: Zone) { this._zoneFrequency = value; 
    for (let i = 0; i < value.length; i++) {
      this.runtime?.setFilterFrequency(i, this.map(value[i].value, 0, 1, 10, 24000), false)
    }
  }
  ...
  private _zoneGain: Zone = [];
  public get zoneGain(): Zone { return this._zoneGain; }
  @property({ type: Object })
  public set zoneGain(value: Zone) {
    this._zoneGain = value;
    for (let i = 0; i < value.length; i++) {
      this.runtime?.setFilterGain(i, this.map(value[i].value, 0, 1, -15, 15), false)
    }
  }

  private _zoneQ: Zone = [];
  public get zoneQ(): Zone { return this._zoneQ; }
  @property({ type: Object })
  public set zoneQ(value: Zone) {
    this._zoneQ = value;
    for (let i = 0; i < value.length; i++) {
      this.runtime?.setFilterQ(i, this.map(value[i].value, 0, 1, 0, 18), false)
    }
  }
